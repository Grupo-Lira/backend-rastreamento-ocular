...
io.on("connection", (socket) => {
  ...
  socket.on("iniciar_experimento_com_config", (config) => {
    ...
    if (!estado || estado.fase_atual !== 0) {
      console.log(
        `Tentativa de iniciar experimento em fase inválida: ${estado?.fase_atual}. cliente: ${socket.id}`
      );
    ...
    if (!Array.isArray(config) || config.length === 0) {
      console.error(
        `Configurações de alvo inválidas recebidas. cliente: ${socket.id}`
      );
  ...
  socket.on("gaze_data", (data) => {
    ...
    try {
      ...
      if (
        !estado ||
        estado.fase_atual === 3 ||
        estado.foco_concluido_nesta_fase
      )
        return;
